...
download_binary() {
    ...
    if [ ! -x "$PROG" ]; then
        echo "[sing-box] Binary not found, downloading ${latest_version:-latest}..."
    ...
    chmod +x "$PROG"
    [ -n "$latest_version" ] && echo "$latest_version" > "$VER_FILE"
    echo "[sing-box] Installed sing-box v${latest_version:-unknown}"
}
